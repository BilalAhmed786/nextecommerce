...
export default function EditProductForm() {
  ...
  const {
    ...
    loading,
    refetch,
  } = useQuery(getsingle_product, { variables: { id } });
  ...
  //preview database gallery image incase from database + if user select files
  function getImagePreview(image: File | dataImages) {
    return image instanceof File ? URL.createObjectURL(image) : image.url;
  }
//remove images if not from database and if from database hit api and remove
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let uploadedGalleryImages: { url: string; publicId: string } | undefined;
  ...
  return (
    <form
      ...
      className="max-w-2xl mt-20 mx-auto p-4 space-y-6 bg-white shadow rounded"
    >
      ...
      <select
        ...
        onChange={handleChange}
        className="w-[35%] border p-2 rounded"
      >
      ...
      <div className="relative">
        <label className="font-bold">Main Image</label>
        <br />
        <input
          type="file"
          accept="image/*"
        ...
        {!mainImageFile ? (
          <img
            src={productData.getSingleproduct?.image}
          ...
          {imagesFiles.map((img, idx) => (
            <div key={idx} className="relative">
              ...
              <button
                className="absolute text-white top-0 bg-amber-600 rounded-full px-1 text-xs cursor-pointer"
                onClick={() =>
                  handleRemoveImage(idx, "id" in img ? img.id : null)
                }
              >
                X
              </button>
      ...
      {valdiation ? (
        <div className="text-red-500 text-center">{valdiation}</div>
      ) : (
        ""
      )}
      <button
